...
const SCENES = [
  {
    ...
    accent: GOLD,
    glow: "radial-gradient(circle at 50% 50%, rgba(200,155,109,0.25), transparent 70%)",
    image: null,
    facts: null,
    cta: false,
  },
  {
    ...
    accent: "#C8A470",
    glow: "radial-gradient(circle at 40% 60%, rgba(232,210,175,0.35), transparent 75%)",
  ...
  },
  {
    ...
    accent: "#B89060",
    glow: "radial-gradient(circle at 60% 40%, rgba(200,168,130,0.3), transparent 70%)",
  ...
  },
  {
    ...
    accent: "#A07850",
    glow: "radial-gradient(circle at 30% 50%, rgba(176,131,90,0.3), transparent 75%)",
  ...
  },
  {
    ...
    accent: "#8A9E80",
    glow: "radial-gradient(circle at 70% 60%, rgba(155,175,142,0.25), transparent 75%)",
  ...
  },
  {
    ...
    accent: GOLD,
    glow: "radial-gradient(circle at 50% 50%, rgba(200,155,109,0.3), transparent 70%)",
    image: null,
    facts: null,
    cta: true,
  },
...
export function Ingredients() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const tl = gsap.timeline({
        scrollTrigger: {
          ...
          start: "top top",
            ...
            let currentFinalX = 0;
            if (lastPanel) {
              currentFinalX = -(lastPanel.offsetLeft - (window.innerWidth - lastPanel.offsetWidth) / 2);
            }
            const dist = lastPanel ? Math.abs(currentFinalX) : (track.scrollWidth - window.innerWidth);
            // Balance scroll length on mobile so it's not too long, but also not shifting too fast
      ...
      if (lastPanel) {
        tl.to(track, { 
          x: () => -(lastPanel.offsetLeft - (window.innerWidth - lastPanel.offsetWidth) / 2), 
          ease: "none", force3D: true 
        }, 0);
        tl.to(spine, { 
          x: () => -(spine.scrollWidth - window.innerWidth) * 0.3, 
          ease: "none", force3D: true 
        }, 0);
        particlesRef.current.forEach((p, i) => {
          if (p) {
            tl.to(p, { 
              x: () => -(lastPanel.offsetLeft - (window.innerWidth - lastPanel.offsetWidth) / 2) * (0.5 + (i % 3) * 0.08), 
              ease: "none", force3D: true 
            }, 0);
          }
        });
  ...
  return (
    <div ref={wrapRef} className="bg-[#F7F4EF] relative">
      <div ref={stickyRef} className="sticky top-0 h-screen w-full overflow-hidden flex flex-col justify-center">
        <div className="absolute inset-0 opacity-[0.05] pointer-events-none z-[1] mix-blend-multiply bg-[url('https://grainy-gradients.vercel.app/noise.svg')]" />

        <div className="absolute inset-0 z-[2] overflow-hidden pointer-events-none opacity-80">
          <div ref={(el) => { meshRefs.current[0] = el; }} className="absolute -top-[10%] -left-[20%] w-[120vw] md:w-[60vw] h-[120vw] md:h-[60vw] rounded-full blur-[80px] md:blur-[100px] bg-[#E8D2BB]/50 md:bg-[#E8D2BB]/20 mix-blend-multiply" />
          <div ref={(el) => { meshRefs.current[1] = el; }} className="absolute bottom-[5%] right-[10%] w-[100vw] md:w-[50vw] h-[100vw] md:h-[50vw] rounded-full blur-[90px] md:blur-[120px] bg-[#D4C3A3]/50 md:bg-[#D4C3A3]/20 mix-blend-multiply" />
          <div ref={(el) => { meshRefs.current[2] = el; }} className="absolute top-[30%] left-[30%] w-[140vw] md:w-[70vw] h-[140vw] md:h-[70vw] rounded-full blur-[110px] md:blur-[140px] bg-[#C89B6D]/30 md:bg-[#C89B6D]/15 mix-blend-multiply" />
        </div>

        <div ref={glowRef} className="absolute inset-0 pointer-events-none z-[3]" />

        <div className="absolute inset-0 pointer-events-none z-[4] overflow-visible w-[800vw]">
          {particles.map((_, i) => (
            <div
              key={i}
        ...
        <div className="absolute top-1/2 left-0 w-[800vw] h-[300px] -translate-y-1/2 pointer-events-none z-[7] opacity-30">
        ...
        <div ref={trackRef} className="relative z-[10] flex h-full items-center pl-[15vw] pr-[15vw] will-change-transform gap-[10vw]">
          {SCENES.map((scene, i) => {
            ...
            if (isCenter) {
              return (
                <div key={scene.id} className="scene-container flex-shrink-0 w-[85vw] md:w-[50vw] h-full flex flex-col items-center justify-center text-center">
                  <div className="card-group w-full flex flex-col items-center justify-center">
                    <SheetRevealText delay={0}>
                      <p className="font-sans text-[10px] md:text-[11px] tracking-[0.35em] uppercase mb-6 md:mb-8" style={{ color: scene.accent }}>{scene.label}</p>
                    </SheetRevealText>
                    <SheetRevealText delay={0.1}>
                      <h2 className={`font-orange text-[clamp(55px,9vw,140px)] leading-[0.9] tracking-[-0.03em] mb-4 md:mb-6 whitespace-pre-line ${scene.id === "cta" ? "italic" : ""}`} style={{ color: DARK }}>{scene.title}</h2>
                    </SheetRevealText>
                    {scene.subtitle && (
                      <SheetRevealText delay={0.2}>
                        <p className="font-serif italic text-[clamp(18px,1.8vw,28px)] text-[#8B7355] mb-6 md:mb-8">{scene.subtitle}</p>
                      </SheetRevealText>
                    )}
                    {scene.body && (
                      <SheetRevealText delay={0.3}>
                        <p className="font-sans text-[14px] md:text-[15px] leading-relaxed text-[#6B5B48] whitespace-pre-line max-w-[90%] md:max-w-md mx-auto">{scene.body}</p>
                      </SheetRevealText>
                    )}
                    {scene.cta && (
                      <SheetRevealText delay={0.4}>
                        <button
                          className="mt-8 md:mt-12 font-sans text-[10px] md:text-[11px] tracking-[0.25em] uppercase text-[#F7F3EE] bg-[#2C2416] px-8 md:px-10 py-3 md:py-4 rounded-full transition-all hover:-translate-y-1 shadow-2xl"
            ...
            return (
              <div key={scene.id} className="scene-container flex-shrink-0 w-[85vw] max-w-[1400px] h-full flex items-center justify-center">
                <div className={`card-group relative w-full flex items-center justify-center flex-col ${imageOnLeft ? "md:flex-row" : "md:flex-row-reverse"}`}>
                  ...
                  <div className={`anim-img-card opacity-0 invisible relative w-[65%] md:w-[40%] aspect-[3/4] z-10 max-md:mb-[-15%] ${imageOnLeft ? "md:-mr-[8%]" : "md:-ml-[8%]"}`} data-side={side}>
                    <div className="w-full h-full rounded-[30px] md:rounded-[40px] overflow-hidden shadow-[0_40px_100px_rgba(0,0,0,0.12)] border border-white/40 bg-[#F4EFE8]">
                      {scene.image && <img src={scene.image} alt={scene.title} className="w-full h-full object-cover" />}
                    </div>
                  </div>

                  <div className="anim-text-card opacity-0 invisible relative w-[90%] md:w-[50%] z-20" data-side={side === "left" ? "right" : "left"}>
                    <div className="bg-white/40 backdrop-blur-2xl p-6 sm:p-8 md:p-16 rounded-[30px] md:rounded-[40px] shadow-[0_40px_120px_rgba(0,0,0,0.08)] border border-white/60">
                      <SheetRevealText delay={0}>
                        <p className="font-sans text-[10px] tracking-[0.35em] uppercase mb-4 md:mb-6" style={{ color: scene.accent }}>{scene.label}</p>
                      </SheetRevealText>
                      <SheetRevealText delay={0.1}>
                        <h3 className="font-orange text-[clamp(40px,8vw,84px)] leading-[0.9] tracking-[-0.02em] mb-3 md:mb-4 text-[#2C2416]">{scene.title}</h3>
                      </SheetRevealText>
                      <SheetRevealText delay={0.2}>
                        <p className="font-serif italic text-[clamp(16px,1.4vw,22px)] text-[#8B7355] mb-4 md:mb-8">{scene.subtitle}</p>
                      </SheetRevealText>
                      <SheetRevealText delay={0.3}>
                        <p className="font-sans text-[13px] md:text-[14px] leading-relaxed text-[#5C4D3C] mb-6 md:mb-10">{scene.body}</p>
                      </SheetRevealText>
                      {scene.facts && (
                            ...
                            <li key={fi} className="flex items-start gap-3">
                              <SheetRevealText delay={0.4 + fi * 0.1}>
                                <span className="flex items-start gap-3 md:gap-4">
                                  ...
                                  <span className="font-sans text-[12px] md:text-[13px] tracking-wide text-[#4A3C2B]">{f}</span>
                                </span>
                              </SheetRevealText>
                            </li>
                          ))}
