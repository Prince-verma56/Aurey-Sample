...
export default function ProductCard({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "group relative flex w-full flex-col overflow-hidden rounded-[24px] border border-stone-100 bg-white shadow-[0_10px_30px_rgba(0,0,0,0.04)]",
        "transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_50px_rgba(139,106,80,0.15)] will-change-transform",
        className
      )}
      initial={
        shouldReduceMotion
    ...
    >
      {/* Image — full bleed */}
      <div className="relative aspect-[4/5] overflow-hidden bg-[#F4EFE8] w-full">
        <img
          alt={title}
          className={cn(
      ...
      {/* Content */}
      <div className="flex flex-1 flex-col p-5 md:p-6 z-10 relative bg-white">
        {subtitle && (
          <p 
            className="text-[9px] md:text-[10px] uppercase tracking-[0.2em] mb-2 font-semibold"
            style={{ color: accentColor || "#8B6A50" }}
          >
            {subtitle}
          </p>
        )}
        
        <h3 className="line-clamp-2 text-lg md:text-xl font-[var(--font-inter)] font-semibold tracking-tight text-[#2C2416] leading-snug mb-2">
          {title}
        </h3>
        ...
        <div className="mt-3 flex items-baseline gap-2">
          <span className="font-semibold text-[#4A3C2B] text-lg tracking-tight">
            {currency}{price}
          </span>
          {hasDiscount && (
            <>
        ...
        <div className="mt-auto pt-5 flex items-center justify-between">
          <span className="text-[10px] uppercase tracking-widest text-[#8B6A50] font-semibold group-hover:text-stone-900 transition-colors hidden md:block">
            Explore &rarr;
          </span>
          <SmoothButton
